...
The code provided in `deck/view.kt` integrates [FileKit](https://github.com/vinceglb/FileKit) to show the file picker of the platform the application is running on. The button are disabled, remember to set `enabled = true` in the call to `IconButton`. Saving the deck is easy, but loading it potentially involves getting the information from each of them. You should use the `getById` method from `PokemonTcgApi` to retrieve the `Card` corresponding to a given identifier.

In a first approximation, using `map` over the sequence of identifiers should be enough. Albeit simple, that solution lacks performance. Arrow provides [_high-level concurrency_](https://arrow-kt.io/learn/coroutines/parallel/) which solves the problem quite succinctly. Use [`parMap`](https://arrow-kt.io/learn/coroutines/parallel/#independently-in-parallel) to turn the sequential iteration into a concurrent set of operations.
...
```admonish tldr title="A good architecture and UI"

You are free to implement the actions anywhere, but we strongly recommend making them part of `DeckViewModel`, and using `viewModelScope.launch` to obtain a coroutine scope in which the action runs. 

However, doing so only ensures that the action runs in the context of our view model, but does not provide any feedback to the user. A better approach would be to introduce actions, as we did in the [_Deck building_](./build.md) section, and state pertaining the progress of saving and loading.
